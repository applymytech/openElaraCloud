...
const _EMBEDDING_DIMENSION = 768; // M2-BERT output dimension
...
function convertConversationToMarkdown(messages: ConversationMessage[], title?: string): string {
	const lines: string[] = [];
 ...
 */
export async function ingestKnowledgeFile(file: File, tags?: string[]): Promise<RAGDocument> {
	const user = auth.currentUser;
	if (!user) {
		throw new Error("Not authenticated");
	}
...
export async function searchRAG(
	...
	const { minSimilarity = 0.3, type = "all", useKeywordFallback = true } = options;

	// Get all RAG documents
	let ragQuery;
	if (type !== "all") {
		ragQuery = await getDocs(query(collection(db, "users", user.uid, "rag"), where("type", "==", type)));
	} else {
		ragQuery = await getDocs(collection(db, "users", user.uid, "rag"));
	}

	const documents: RAGDocument[] = ragQuery.docs.map(
		(doc) =>
			({
				id: doc.id,
				...doc.data(),
	...
	logger.debug(`searchRAG found ${documents.length} documents (type: ${type})`, { component: "RAG" });

	if (documents.length === 0) {
		console.warn("[RAG] No documents in RAG database - user manual may not be ingested yet");
		return [];
	}
 ...
 */
export async function hybridSearchRAG(queryText: string, maxResults: number = 5): Promise<RAGSearchResult[]> {
	const user = auth.currentUser;
	if (!user) {
		throw new Error("Not authenticated");
	}

	const ragQuery = await getDocs(collection(db, "users", user.uid, "rag"));
...
export async function buildRAGContext(userMessage: string): Promise<string | null> {
	try {
		...
		const contextParts: string[] = ["## Relevant Context from Your Knowledge Base\n"];
		...
		logger.debug(`Built context: ${results.length} docs, ${context.length} chars`, { component: "RAG" });
		return context;
	} catch (e) {
...
export async function listRAGDocuments(): Promise<RAGDocument[]> {
	...
	const ragQuery = await getDocs(query(collection(db, "users", user.uid, "rag"), orderBy("updatedAt", "desc")));

	return ragQuery.docs.map(
		(doc) =>
			({
				id: doc.id,
				...doc.data(),
