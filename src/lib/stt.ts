...
export async function startRecording(): Promise<{ success: boolean; error?: string }> {
	...
	try {
		...
		logger.debug(`Recording started with mimeType: ${mimeType}`, { component: "STT" });
		return { success: true };
	} catch (error: any) {
...
export async function stopRecording(): Promise<{ success: boolean; audioBlob?: Blob; error?: string }> {
	...
	return new Promise((resolve) => {
		mediaRecorder!.onstop = () => {
			...
			logger.debug(`Recording stopped, blob size: ${audioBlob.size} bytes`, { component: "STT" });
...
export function cancelRecording(): void {
	if (mediaRecorder && isRecording) {
		...
		audioChunks = [];
		isRecording = false;
		logger.debug("Recording cancelled", { component: "STT" });
	}
}
 ...
 */
export async function transcribeAudio(audioBlob: Blob, options: STTOptions = {}): Promise<STTResult> {
	...
	try {
		...
		logger.debug("Sending audio to Whisper API...", { component: "STT" });
		...
		logger.debug(`Transcription complete: ${result.text?.length || 0} chars`, { component: "STT" });
 ...
 * 3. Return text
 */
export async function recordAndTranscribe(_options: STTOptions = {}): Promise<STTResult> {
