...
export async function exportDocument(content: string, options: ExportOptions): Promise<void> {
	...
	switch (format) {
		case "markdown":
			exportContent = includeMetadata ? addMarkdownMetadata(content, metadata) : content;
...
function addMarkdownMetadata(content: string, metadata?: DocumentMetadata): string {
	...
	if (metadata.created) {
		frontmatter += `date: "${metadata.created.toISOString()}"\n`;
	}
	if (metadata.aiModel) {
		frontmatter += `ai_model: "${metadata.aiModel}"\n`;
	}
 ...
 */
async function convertToHTML(content: string, metadata?: DocumentMetadata, includeMetadata?: boolean): Promise<string> {
