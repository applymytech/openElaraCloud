...
import { chat, type ChatMessage, type Tool } from './api';
import { executeToolCall, getAvailableTools, type DeepThoughtConfig, type ToolResult } from './tools';
...
function buildDeepThoughtSystemPrompt(userQuery: string, maxTurns: number): string {
  ...
  const unavailableWarning = unavailableReasons.length > 0
...
2. **MIDDLE TURNS - Work**: Use tools to gather information, create images/videos, etc.
   ...
   - make_image: Generate images (YOU choose model based on style)
   - make_video: Generate videos
...
export async function executeDeepThought(
  userQuery: string,
  options: DeepThoughtOptions
): Promise<DeepThoughtResult> {
  const { maxTurns, systemPrompt, onProgress } = options;
  ...
  try {
    ...
    while (config.currentTurn < maxTurns) {
      ...
      const response = await chat(messages, {
        model: 'meta-llama/Llama-3.3-70B-Instruct-Turbo',  // Good for reasoning
      ...
      const isAcknowledgment = config.currentTurn === 1 && 
                               !assistantMessage.tool_calls &&
                               (assistantMessage.content?.length || 0) < 200;
      ...
      if (assistantMessage.content && !assistantMessage.tool_calls) {
        ...
        const isComprehensive = assistantMessage.content.length > 300 || 
