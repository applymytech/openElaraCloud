...
import type { ContentMetadata, SignedContent } from "./signing";
...
export async function storeMedia(
	...
	if (quota.used + blob.size > quota.limit) {
		throw new Error(
			`Storage quota exceeded. Used: ${formatBytes(quota.used)}, Limit: ${formatBytes(quota.limit)}. Please download and delete some files.`,
		);
	}
...
export async function downloadMediaWithSidecar(mediaId: string): Promise<{
	...
	const [mediaUrl, sidecarUrl] = await Promise.all([getDownloadURL(mediaRef), getDownloadURL(sidecarRef)]);

	const [mediaResponse, sidecarResponse] = await Promise.all([fetch(mediaUrl), fetch(sidecarUrl)]);
...
export async function deleteMedia(mediaId: string): Promise<void> {
	...
	try {
		const mediaRef = ref(storage, `${media.storageUrl}/${media.filename}`);
		const sidecarRef = ref(storage, `${media.storageUrl}/${media.filename}.metadata.json`);
		await Promise.all([deleteObject(mediaRef), deleteObject(sidecarRef)]);
	} catch (e) {
...
export async function storeMediaFromUrl(
	...
	const metaContentType: "image" | "video" | "audio" = contentType.startsWith("video")
		? "video"
...
export async function getStorageStatus(): Promise<StorageStatus> {
	...
	const downloadRecommended = mediaFiles.some((m) => !m.downloaded || m.createdAt < sevenDaysAgo);
