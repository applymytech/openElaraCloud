			...
			<div className="power-knowledge-panel" onClick={(e) => e.stopPropagation()}>
				...
				{!configured ? (
					<div className="pk-not-configured">
						...
						<p>
							Add your Exa.ai API key in <strong>Settings → API Keys</strong> to enable this feature.
						</p>
					...
					<div className="pk-content">
						...
						{activeTask && (
							<div className="pk-query-section">
								...
								<button className="pk-advanced-toggle" onClick={() => setShowAdvanced(!showAdvanced)}>
									...
									<div className="pk-advanced-options">
										<div className="pk-option-row">
											...
											<input
												...
												max="20"
												value={numResults}
												onChange={(e) => setNumResults(parseInt(e.target.value, 10) || 5)}
												disabled={loading}
											/>
										</div>
										<div className="pk-option-row">
								...
								<div className="pk-actions">
									<button className="pk-btn pk-btn-primary" onClick={handleExecute} disabled={loading || !query.trim()}>
										{loading
											? activeTask === "research"
									...
									<button className="pk-btn pk-btn-secondary" onClick={handleClear} disabled={loading}>
										Clear
									</button>
								</div>
								...
								{loading && activeTask === "research" && (
									<div className="pk-progress">
										<div className="pk-progress-bar" style={{ width: `${researchProgress}%` }} />
									</div>
								)}
							</div>
						)}
									...
									<div className="pk-search-results">
										{result.results.map((r, i) => (
											<div key={i} className="pk-result-item">
												...
												<a href={r.url} target="_blank" rel="noopener noreferrer" className="pk-result-url">
													{r.url}
												</a>
												{r.score && <span className="pk-result-score">Score: {r.score.toFixed(3)}</span>}
												{r.highlights && r.highlights.length > 0 && (
													<div className="pk-result-highlight">"{r.highlights[0]}"</div>
												)}
											</div>
										))}
									</div>
