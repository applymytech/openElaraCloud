...
const BUILTIN_CHAT_PROMPTS: Prompt[] = [
	{
		...
		type: "chat",
		content:
			"You are a creative writer with a vivid imagination. Help users develop stories, poems, and creative content.",
		isBuiltIn: true,
	},
	{
		id: "chat_code_helper",
...
export default function PromptManager({ isOpen, onClose, onSelectPrompt, filterType = "all" }: PromptManagerProps) {
	...
	// Filter prompts
	const filteredPrompts = filter === "all" ? allPrompts : allPrompts.filter((p) => p.type === filter);

	const handleAddPrompt = () => {
		if (!newPrompt.name || !newPrompt.content || !newPrompt.type) {
			return;
		}
		...
		<div className="prompt-manager-overlay" onClick={onClose}>
			<div className="prompt-manager-modal" onClick={(e) => e.stopPropagation()}>
				...
				<div className="pm-tabs">
					<button className={`pm-tab ${filter === "all" ? "active" : ""}`} onClick={() => setFilter("all")}>
						All
					</button>
					<button className={`pm-tab ${filter === "chat" ? "active" : ""}`} onClick={() => setFilter("chat")}>
						💬 Chat
					</button>
					<button className={`pm-tab ${filter === "image" ? "active" : ""}`} onClick={() => setFilter("image")}>
						📸 Image
					</button>
					<button className={`pm-tab ${filter === "video" ? "active" : ""}`} onClick={() => setFilter("video")}>
						🎬 Video
					</button>
				</div>
					...
					{filteredPrompts.map((prompt) => (
						...
						>
							<div className="pm-prompt-header">
								...
								<span className={`pm-prompt-type type-${prompt.type}`}>{prompt.type}</span>
							</div>
