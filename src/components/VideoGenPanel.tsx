...
import { 
  generateVideo, 
  generateAgenticVideo,
  VIDEO_MODELS,
  type GeneratedVideo,
  type AgenticVideoResult,
} from '../lib/mediaGeneration';
import { getActiveCharacter } from '../lib/characters';
import { getMoodState } from '../lib/mood';
import { storeMediaFromUrl, deleteMedia, type StoredMedia } from '../lib/storage';

// Use the exported VIDEO_MODELS constant
const VIDEO_MODEL_METADATA = VIDEO_MODELS;
...
export default function VideoGenPanel({ onClose, onVideoGenerated, conversationContext }: VideoGenPanelProps) {
    ...
    try {
      ...
      try {
        ...
        const filename = mode === 'selfie' 
          ? `${character.name}_video_${timestamp}.mp4`
          : `video_${timestamp}.mp4`;
          
        const storedFile = await storeMediaFromUrl(
          video.url,
          filename,
          'video/mp4',
          {
  ...
  const handleCut = async () => {
    if (!storedMediaId || !result?.url) return;
  ...
  return (
    <div className="video-gen-panel">
      ...
      <div className="cost-warning">
        ⚠️ Video generation uses more API credits than images. Duration affects cost.
      </div>
      ...
      {result && (
        <div className="result-container">
          ...
          <div className="result-info">
            <span>📹 {result.duration}s • {result.resolution}</span>
          </div>
