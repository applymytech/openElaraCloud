        ...
        {toolResults.map((result, idx) => (
          <div key={idx} className={`tool-result ${result.success ? 'success' : 'failure'}`}>
            <div className="tool-result-header">
              <span className="tool-name">{getToolIcon(result.tool)} {getToolDisplayName(result.tool)}</span>
            ...
            {!result.success && (
              <div className="tool-result-error">
                ❌ Error: {result.output?.error || 'Tool execution failed'}
              </div>
            )}
          </div>
        ))}
...
function getToolIcon(toolName: string): string {
  switch (toolName) {
    ...
    case 'read_url': return '📄';
    case 'make_image': return '🎨';
    case 'make_video': return '🎬';
    case 'save_thought': return '💭';
    default: return '🔧';
  }
}
...
function getToolDisplayName(toolName: string): string {
  switch (toolName) {
    ...
    case 'make_image': return 'Generate Image';
    case 'make_video': return 'Generate Video';
    case 'save_thought': return 'Save Thought';
...
function renderToolOutput(result: ToolResult) {
  ...
  switch (tool) {
    ...
    case 'read_url':
      return (
        <div>
          <strong>URL:</strong> <a href={output.url} target="_blank" rel="noopener noreferrer">{output.url}</a>
    ...
    case 'make_image':
      return (
        <div>
          <div><strong>Prompt:</strong> {output.description}</div>
          <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.6)' }}>Model: {output.model}</div>
          {output.imageUrl && (
            <img src={output.imageUrl} alt="Generated" className="tool-image-preview" />
          )}
        </div>
      );
    ...
    case 'make_video':
      return (
        <div>
          <div><strong>Scene:</strong> {output.description}</div>
          <div style={{ fontSize: '12px', color: 'rgba(255,255,255,0.6)' }}>
            Model: {output.model} | AI Decision: {output.aiDecision}
          </div>
          {output.videoUrl && (
            <video src={output.videoUrl} controls className="tool-video-preview" />
          )}
        </div>
      );
    ...
    case 'save_thought':
      return (
        <div className="tool-thought-content">
          "{output.saved}"
        </div>
      );
