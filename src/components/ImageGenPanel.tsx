...
import { type GeneratedImage, generateAgenticSelfie, generateImage, IMAGE_MODELS } from "../lib/mediaGeneration";
import { getMoodState } from "../lib/mood";
import { downloadWithMetadata } from "../lib/signing";
import { cutMedia, storeMedia } from "../lib/storage";
...
export default function ImageGenPanel({ onClose, onImageGenerated, conversationContext }: ImageGenPanelProps) {
	...
	// Use first available model (FLUX.1-schnell is fast and affordable)
	const firstAvailable = Object.keys(IMAGE_MODELS)[0] || "black-forest-labs/FLUX.1-schnell";
	...
	return (
		<div className="image-gen-panel">
			...
			<div className="mode-toggle">
				<button type="button" className={`mode-btn ${mode === "selfie" ? "active" : ""}`} onClick={() => setMode("selfie")}>
					📸 {character.name} Selfie
				</button>
				<button type="button" className={`mode-btn ${mode === "custom" ? "active" : ""}`} onClick={() => setMode("custom")}>
					🎨 Custom Image
				</button>
			</div>
			...
			<div className="form-group">
				...
				<select id="image-model-select" value={model} onChange={(e) => setModel(e.target.value)} className="nexus-input">
			...
			<div className="form-group">
				<label htmlFor="image-prompt-input">
					{mode === "selfie" ? `Scene Suggestion (optional) - ${character.name} will decide the final pose` : "Prompt"}
				</label>
				<textarea
			...
			{mode === "selfie" && (
				<div className="character-preview">
					<div className="preview-header">
						{character.iconPath ? (
							<Image src={character.iconPath} alt={character.name} className="preview-icon" width={32} height={32} />
						) : (
			...
			{aiDecision && !generating && (
				<div className="ai-decision">
					<div className="ai-decision-header">
						{character.iconPath ? (
							<Image src={character.iconPath} alt={character.name} className="ai-decision-icon" width={32} height={32} />
						) : (
			...
			{result && (
				<div className="result-container">
					...
					<img src={result.signedContent.dataUrl} alt="Generated" className="result-image" />

					{/* Storage Status */}
					{storedMediaId && <div className="storage-status">☁️ Saved to cloud storage</div>}

					<div className="result-actions">
						<button type="button" onClick={handleCut} className="nexus-btn nexus-btn-primary">
							📥 Download & Clear
						</button>
						<button type="button" onClick={handleDownloadDirect} className="nexus-btn nexus-btn-secondary">
							💾 Download Only
						</button>
					</div>
